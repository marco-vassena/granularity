...
mutual
  ...
  cg2fg ≈θ (⌞_⌟ᵀ {Σ = Σ}  ⟨ ≈Σ , ≈μ ⟩)  (New {pc = pc} x₁ pc⊑ℓ) =
    let
       M = Σ pc
       ≈M = ≈Σ pc
       r ∧ ≈r ∧ x₁' = cg2fgᴾ _ pc ≈θ x₁
       c = ⟨ Σ FG.[ pc ↦ FG.snocᴹ M r ]ˢ , _ , Refᴵ pc FG.∥ M ∥ᴹ ^ pc ⟩
       ≈c = ⟨ ⟨ update-↓≈ˢ ≈Σ (new-↓≈ᴹ ≈M ≈r) , ≈μ ⟩ , Ref′ pc ∥ ≈M ∥-↓≈ᴹ ⟩
       ⇓c = New x₁'
    in c ∧ ≈c  ∧ ⇓c
  ...
  cg2fg ≈θ ⌞ ⟨ ≈Σ , ≈μ ⟩ ⌟ᵀ (Write {pc = pc} x₁ x₂ pc⊑ℓ ℓ₁⊑ℓ M≔) with cg2fgᴾ _ pc ≈θ x₁ | cg2fgᴾ _ pc ≈θ x₂
  ... | Refᴵ .ℓ .n ∧ Refᴵ ℓ n ∧ x₁' | _ ∧ ≈r ∧ x₂'
    with write-↓≈ᴹ (≈ᴿ-⊑ ≈r pc⊑ℓ) M≔ (≈Σ ℓ)
  ... | M ∧ M≔' ∧ ≈M = c ∧ ≈c ∧ ⇓c
    where c = ⟨ _ , _ , （） ^ pc ⟩
          ≈c = ⟨ ⟨ update-↓≈ˢ ≈Σ ≈M , ≈μ ⟩ , （） ⟩
          ⇓c = Write x₁' pc⊑ℓ x₂' pc⊑ℓ M≔'

  cg2fg ≈θ ⌞ ≈ᴾ ⌟ᵀ (LabelOfRef {pc = pc} x refl) with cg2fgᴾ _ pc ≈θ x
  ... | Refᴵ .ℓ .n ∧ Refᴵ ℓ n ∧ x' = ⟨ _ , _ , ⌞ ℓ ⌟ ^ (pc ⊔ ℓ) ⟩ ∧ ⟨ ≈ᴾ , ⌞ ℓ ⌟ ⟩ ∧ (LabelOfRef x' (sym-⊔ pc ℓ))

  cg2fg ≈θ (⌞_⌟ᵀ {μ = μ} ⟨ ≈Σ , ≈μ ⟩) (New-FS {pc = pc} x pc⊑ℓ) =
    let r ∧ r≈ ∧ x' = cg2fgᴾ _ pc ≈θ x
        c = ⟨ _ , FG.snocᴴ μ (r ^ pc) , (Refˢ FG.∥ μ ∥ᴴ) ^ pc ⟩
        ≈c = ⟨ ⟨ ≈Σ , new-↓≈ᴴ ≈μ ⌞ r≈ ⌟ᴸ ⟩ , Refˢ′ ∥ ≈μ ∥-↓≈ᴴ ⟩
        c⇓ = New-FS x'
    in c ∧ ≈c ∧ c⇓

  cg2fg ≈θ ⌞ ≈ᴾ ⌟ᵀ (Read-FS {pc = pc} x n∈μ refl) with cg2fgᴾ _ pc ≈θ x
  ... | Refˢ .n ∧ Refˢ n ∧ x' with lookup-↓≈ᴴ n∈μ (heap-↓≈ᴴ ≈ᴾ)
  ... | r ^ ℓ ∧ n∈M' ∧ ⌞ ≈r ⌟ᴸ
    = ⟨ _ , _ , r ^ (pc ⊔ ℓ) ⟩ ∧ ⟨ ≈ᴾ , ≈ᴿ-⊑ ≈r (join-⊑₂ _ _)  ⟩ ∧ Read-FS x' n∈M' refl

  cg2fg ≈θ ⌞ ⟨ ≈Σ , ≈μ ⟩ ⌟ᵀ (Write-FS {pc = pc} x₁ x₂ n∈μ ⊑₁ _ w) with cg2fgᴾ _ pc ≈θ x₁ | cg2fgᴾ _ pc ≈θ x₂
  ... | Refˢ .n ∧ Refˢ n ∧ x₁' | _ ∧ ≈r ∧ x₂'
    with lookup-↓≈ᴴ n∈μ ≈μ
  ... | r′ ∧ n∈μ′ ∧ ⌞ r≈′ ⌟ᴸ =
    let μ ∧ w' ∧ ≈μ′ = write-↓≈ᴴ ⌞ ≈r ⌟ᴸ w ≈μ
        c = ⟨ _ , _ , （） ^ pc ⟩
        ≈c = ⟨ ⟨ ≈Σ , ≈μ′  ⟩ , （） ⟩
        pc⊔pc = sym (idem-⊔ pc)
        ⇓c = Write-FS x₁' x₂' n∈μ′ ⊑₁ pc⊔pc w' -- x₁'
    in c ∧ ≈c ∧ ⇓c

  cg2fg ≈θ ⌞ ≈ᴾ ⌟ᵀ (LabelOfRef-FS {pc = pc} x n∈μ refl) with cg2fgᴾ _ pc ≈θ x
  ... | Refˢ .n ∧ Refˢ n ∧ x'  with lookup-↓≈ᴴ n∈μ (heap-↓≈ᴴ ≈ᴾ)
  ... | r ^ ℓ ∧ n∈μ′ ∧ ⌞ r≈′ ⌟ᴸ
    = ⟨ _ , _ , ⌞ ℓ ⌟ ^ (pc ⊔ ℓ) ⟩ ∧ ⟨ ≈ᴾ , ⌞ ℓ ⌟ ⟩ ∧ (LabelOfRef-FS x' n∈μ′ refl)
